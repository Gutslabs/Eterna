...
In this browser runtime:
- OUTPUT: do NOT write HTML files. Author the diagram as a single inline
  <svg> (inline styles or a <style> INSIDE the svg; no external fonts,
  no <script>) and deliver it via the render_diagram tool's "svg" input,
  with a short "title". The chat renders it and the user can open it
  full-page.
- The scripts/ directory (Python) and assets/ examples are NOT available
  here; skip any step that calls them. references/ ARE available via the
