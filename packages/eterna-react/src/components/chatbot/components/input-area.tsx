...
const PROVIDER_ORDER = ["Codex", "Claude", "Gemini", "Grok", "Other"] as const;
type ProviderName = (typeof PROVIDER_ORDER)[number];

/**
 * Bring any stored value onto a model the picker actually offers, at a
 * reasoning level that model accepts.
 */
function migrateModelValue(value: string): string {
  return normalizeModelSelection(migrateLegacyModelValue(value));
}

/**
 * Expand each model into one submenu per reasoning level. Models that do no
 * reasoning stay a single flat row — an empty submenu would be a dead end.
 */
function withReasoningOptions(
  models: Array<{ name: string; value: string }>,
): ModelEntry[] {
  return models.flatMap(({ name, value }) => {
    const levels = reasoningLevelsFor(value);
    if (levels.length === 0) return [{ name, value }];

    const fallback = defaultReasoningLevelFor(value);
    return levels.map((level) => ({
      name: `${name} · ${REASONING_LEVEL_LABELS[level]}`,
      value: composeModelSelection(value, level),
      group: name,
      optionName: REASONING_LEVEL_LABELS[level],
      optionDescription: level === fallback ? "Default" : undefined,
    }));
  });
}
...
export function DefaultInputArea({
  ...
  // Server-side (Eterna) models: API-fetched or prop fallback
  ...
  // Codex via the local CLIProxyAPI proxy, on the user's ChatGPT subscription.
  const codexGatewayEntries = useMemo(
    () =>
  ...
  // Claude via the same proxy, on the user's Claude Code subscription.
  ...
  const geminiGatewayEntries = useMemo(
    () =>
      withReasoningOptions([
        { name: "Gemini 3.7 Flash", value: "gemini-3.7-flash-high" },
      ]),
    [],
  );
  // Grok via the same local OAuth proxy (xAI / Grok Build account).
  const grokGatewayEntries = useMemo(
    () => withReasoningOptions([{ name: "Grok 4.6", value: "grok-4.6" }]),
    [],
  );
  ...
  useEffect(() => {
    ...
    const normalizedAiModel = aiModel ? migrateModelValue(aiModel) : undefined;
    const normalizedDefaultModel = defaultModel
      ? migrateModelValue(defaultModel)
      : undefined;
  ...
  const effectiveModels = useMemo(() => {
    ...
    const currentModel = migrateModelValue(settings.aiModel?.trim() ?? "");
  ...
  const resolvedDefaultModel = useMemo(() => {
    const candidates = [
      migrateModelValue(settings.defaultModel?.trim() ?? ""),
      migrateModelValue(settings.aiModel?.trim() ?? ""),
  ...
  const handleModelChange = useCallback(
    (newModel: string) => {
      ...
      // Server (Eterna) model selected → switch to proxy mode
  ...
  return (
    <div className={cn("p-3", className)} {...props}>
      ...
      <PromptInput
        ...
        className="mt-0 divide-y-0 rounded-2xl border border-border/80 bg-card shadow-lg shadow-black/20 transition-colors focus-within:border-foreground/25"
        globalDrop
        multiple
      >
        <PromptInputBody>
        ...
        <PromptInputToolbar>
          ...
          {/* Submit/Stop Button — never shrinks, never scrolls away */}
