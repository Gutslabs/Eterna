...
// Module-level defaults: a `{}` literal default parameter would be a fresh
// object every render, which recreates the components/theme context values on
// each streaming commit and re-renders every consumer despite their memos.
const EMPTY_COMPONENTS: NonNullable<ChatbotProviderProps["components"]> = {};
const EMPTY_SLOTS: NonNullable<ChatbotProviderProps["slots"]> = {};
const EMPTY_THEME: NonNullable<ChatbotProviderProps["theme"]> = {};
 ...
 * @example
 * ```tsx
 * const agent = Eterna.create({ model, tools });
 *
 * // Basic usage
 * <Chatbot agent={agent} />
 *
