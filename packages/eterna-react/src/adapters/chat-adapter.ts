...
import { generateId } from "@eterna/core";
...
/**
 * ChatAdapter converts AgentEvents from @eterna/core into UIMessages
...
export class ChatAdapter {
  ...
  processEvent(event: AgentEvent): void {
    switch (event.type) {
      ...
      case "content_delta":
        ...
        // This mirrors eterna's behavior where each model response after tool
  ...
  private updateCurrentAssistantMessage(
    ...
    // Hot path: the streaming target is virtually always the tail message —
    // swap one element on a plain copy instead of running the comparison
    // closure over the whole conversation on every delta.
    const messages = this.state.messages;
    const lastIndex = messages.length - 1;
    const last = messages[lastIndex];
    if (last !== undefined && last.id === currentId) {
