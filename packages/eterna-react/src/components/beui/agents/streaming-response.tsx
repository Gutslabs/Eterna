...
export interface StreamingResponseProps {
  ...
  showActions?: boolean;
  /**
   * LOCAL ADDITION (not upstream): set false when `children` already carries its
   * own prose styling. The built-in rules target descendants (`[&_pre]`,
   * `[&_code]`), so they outrank the classes a markdown renderer puts on those
   * elements — here that turned wrapping code blocks into scrolling ones and
   * dropped the padding a copy button sits in. Re-check on registry updates.
   */
  prose?: boolean;
  className?: string;
...
/** Typography applied to raw children. Skipped when `prose` is false. */
const PROSE_CLASS =
  ...
  return (
    ...
    >
      <div
        ...
        className={cn(prose && PROSE_CLASS, contentClassName)}
      >
        {children}
      </div>
