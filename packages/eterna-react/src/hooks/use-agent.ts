/**
 * useAgent - Generic hook for creating and managing Eterna agent instances
 *
...
import type { AppSettings } from "@eterna/core";
import {
  ContextManager,
...
export interface UseAgentOptions {
  ...
  agentOptions?: Partial<Parameters<typeof Eterna.create>[0]>;
}
...
/**
 * Create and manage an Eterna agent instance
 *
 * @example
 * ```typescript
...
export function useAgent({
  ...
  tools = [],
  instructions,
  name = "Eterna Assistant",
  maxTurns = 2000,
  agentOptions = {},
}: UseAgentOptions): UseAgentReturn {
  const [agent, setAgent] = useState<Eterna | undefined>(undefined);
