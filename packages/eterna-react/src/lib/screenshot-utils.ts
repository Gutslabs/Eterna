...
export const ETERNA_SCREENSHOT_URL_PREFIX =
  "https://eterna-screenshot.invalid/";
 ...
 * with the special eterna-screenshot.invalid URL prefix.
 *
 * Supported formats:
 * - [[screenshot:screenshot_123_abc]]  → ![](https://eterna-screenshot.invalid/screenshot_123_abc)
  ...
  return text.replace(
    ...
    (match: string, content: string) => {
      ...
      if (isValidScreenshotUid(trimmed)) {
        return `![](${ETERNA_SCREENSHOT_URL_PREFIX}${trimmed})`;
      }
      ...
      if (
        ...
        if (uid && isValidScreenshotUid(uid)) {
          return `![](${ETERNA_SCREENSHOT_URL_PREFIX}${uid})`;
        }
      }
