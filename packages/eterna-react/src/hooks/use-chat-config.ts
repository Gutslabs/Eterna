...
export interface UseChatConfigOptions {
  ...
  /** Storage adapter for persisting settings (KeyValueStorage from @eterna/core) */
...
export function useChatConfig(
  ...
  const persistSettings = useCallback(
    async (updates: Partial<AppSettings>): Promise<AppSettings | null> => {
      try {
  ...
  const updateSetting = useCallback(
    async <K extends keyof AppSettings>(
      ...
      settingsRef.current = next;
      setSettings(next);
      const persisted = await persistSettings({ [key]: value });
      if (persisted) {
  ...
  }, [initialSettings, storageAdapter]);
