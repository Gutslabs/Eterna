...
// Memoized on part identity: tool inputs never change once the call is made,
// so streaming re-renders of the surrounding rail/message must not re-run the
// JSON.stringify + Prism pass.
export const ToolInput = memo(function ToolInput({
  className,
  input,
  ...props
}: ToolInputProps) {
  const code = useMemo(() => JSON.stringify(input, null, 2), [input]);
  return (
