...
async function applyLRU(): Promise<void> {
  if (!db) return;
  // count() + a key cursor over the timestamp index delete the oldest entries
  // without ever materializing payloads — getAll() would deserialize up to 50
  // full base64 screenshots (tens of MB) on every save just to read timestamps.
  const excess = await new Promise<number>((res, rej) => {
    const tx = db!.transaction([STORE_NAME], "readonly");
    const req = tx.objectStore(STORE_NAME).count();
    req.onsuccess = () => res(req.result - MAX_SCREENSHOTS);
  ...
  await new Promise<void>((res, rej) => {
    ...
    const cursorReq = delStore.index("timestamp").openKeyCursor();
    cursorReq.onsuccess = () => {
