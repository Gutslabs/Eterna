# Sidepanel 入口链路：new-eterna vs eterna 缺口分析

> **目的**：从 sidepanel 入口出发，系统梳理 `new-eterna` 相比 `eterna` 在端到端链路上的不足之处，为后续补齐工作提供可执行清单。
>
> **生成日期**：2026-02-15
...
| 项目 | 根路径 | 入口文件 |
|------|--------|----------|
| eterna（旧版，全功能） | `eterna/` | `src/sidepanel.tsx` → `ChatBot`（2800+ 行单体组件） |
| new-eterna（新版，模块化） | `new-eterna/packages/` | `browser-ext/src/pages/sidepanel/index.tsx` → `app-root.tsx` → `ChatBot`（组件化 + hooks） |
...
| 阶段 | eterna (旧版) | new-eterna (新版) | 差异 |
|------|-------------|-----------------|------|
| **触发 → 打开** | `chrome.action.onClicked` / `open-eterna` 快捷键 / content script `open-sidepanel` / `openWithPrompt` / `REPLAY_USER_MANUAL` | `chrome.action.onClicked` / `open-eterna` 快捷键 / content script 无 `open-sidepanel` / `openWithPrompt` | 缺少 `REPLAY_USER_MANUAL` 外部触发 |
...
| **Agent 创建** | `new MessageHandler(config)` — 含 `DefaultBackgroundContextManager` + `getAllTools()` + `filterToolsByMode` | `useAgent({ settings, modelFactory, storage, contextProviders, tools })` → `Eterna.create()` | 架构不同但功能对等 |
...
| **Pending Prompt** | `useEffect` 检查 `eterna-pending-prompt` → `setInput()` | `usePendingPrompt()` hook → `initialInput` prop | 一致 |
...
| **Conversation 呼吸边框** | heartbeat → `eterna-conversation-active` → content script 边框 | 同（`useConversationHeartbeat`） | 一致 |
| **流式输出** | `MessageHandler._startStream` — 手动 fetch + `StreamingParser` | `Eterna.chat()` → `AsyncGenerator<AgentEvent>` → `ChatAdapter` | 新版架构更清晰 |
...
| **证据** | 旧版 `eterna/src/background.ts` L3151-3199（`REPLAY_USER_MANUAL`）；`eterna/src/lib/components/chatbot/index.tsx` L1373-1424（`NAVIGATE_AND_SETUP_REPLAY` listener）。新版 `rg "REPLAY_USER_MANUAL\|NAVIGATE_AND_SETUP_REPLAY" new-eterna/` → 零匹配。 |
...
| **证据** | `new-eterna/migration/LOGIC_INCONSISTENCIES_BY_PACKAGE.md` 第 4.2 节。 |
| **影响** | |

| 缺失服务 | 旧版路径 | 影响范围 |
|----------|---------|---------|
| `version-checker.ts` | `eterna/src/lib/services/version-checker.ts` | 无法检测扩展新版本并通知用户 |
| `web-auth.ts` | `eterna/src/lib/services/web-auth.ts` | 旧版 Cookie-based auth 辅助函数缺失（新版有 `AuthProvider` 替代，但功能不完全对等） |
| `recording-upload.ts` | `eterna/src/lib/services/recording-upload.ts` | 无法将录屏上传到托管服务 |
| `screenshot-upload.ts` | `eterna/src/lib/services/screenshot-upload.ts` | 无法将截图上传到托管服务 |
| `user-manuals-api.ts` | `eterna/src/lib/services/user-manuals-api.ts` | 无法从服务端获取用户手册数据 |
| `replay-controller.ts` | `eterna/src/lib/services/replay-controller.ts` | 无法执行步骤回放控制 |
...
| **证据** | `new-eterna/migration/LOGIC_INCONSISTENCIES_BY_PACKAGE.md` 第 2.7 节。新版 `packages/eterna-react/src/components/voice/VoiceInput.tsx` 和 `packages/browser-ext/src/lib/browser-chat-input-area.tsx` 仅使用 Web Speech。 |
| **影响** | 非英语语言识别准确率下降；BYOK 用户无法使用 ElevenLabs 高精度识别。 |
| **优先级** | **P1** |
| **建议** | 在 `packages/eterna-react/src/lib/voice/` 中恢复 ElevenLabs + Server STT 路径。 |
...
| **证据** | 旧版 `eterna/src/background.ts` L1082-1089；新版 `new-eterna/packages/browser-ext/src/background.ts` L31-39。 |
| **影响** | 新用户安装后无引导体验。 |
| **优先级** | **P2** |
...
| **证据** | `rg "ErrorBoundary" new-eterna/` → 零匹配。旧版同样缺少，但旧版组件更少、更稳定。 |
...
| **证据** | 旧版 `eterna/src/lib/components/chatbot/index.tsx` L1313-1325（`skillManager.subscribe`）。新版 `app-root.tsx` 和 `chatbot.tsx` 中无 skill 事件订阅。 |
...
| **证据** | 旧版 `eterna/src/lib/components/chatbot/index.tsx` 中存在 `handleShareConversation`（L1744-1793）和 `handleSaveAsSkill`（L1796-1798）。新版 `browser-chat-header.tsx` 中无对应功能。 |
...
- `aiToken` 存储在 `chrome.storage.local`，UI 中未展示明文。新版 `Eterna.sanitizeErrorMessage()` 内建 Bearer token / API key 脱敏。**状态：安全。**
